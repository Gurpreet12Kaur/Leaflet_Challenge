...
// Define the chooseColor function
function chooseColor(depth) {
    // Define depth ranges and corresponding colors
    var depthRanges = [0, 10, 30, 50, 70, 90];
    var colors = ["#00FF00", "#7FFF00", "#FFFF00", "#FFD700", "#FFA500", "#FF0000"];
    
    // Iterate through the depthRanges to find the appropriate color
    for (var i = 0; i < depthRanges.length; i++) {
        if (depth <= depthRanges[i]) {
            return colors[i];
        }
    }
    // If depth exceeds the maximum range, return a default color
    return "#000000"; // You can change this color as desired
}
